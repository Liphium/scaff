...
func (rp *RectangleProps) Padding(padding scaffui.Padding) {
...
func Rectangle(create func(t *scaff.Tracker, props *RectangleProps)) scaffui.NodeBuilder {
	return scaffui.UseSingleNode("rectangle", create, func(core *scaffui.SingleChildConstruct[RectangleProps]) {
		...
		core.Layout(func(node *scaffui.SingleChildNode[RectangleProps]) (scaffui.Size, error) {
			props := core.Props()
			spec := uispec.SingleChildBoxSpec{
				Parent:  node.Constraints(),
				Wanted:  props.wantedConstraints,
				Padding: props.padding.Or(scaffui.Pad(0)),
			}
		...
		core.Draw(func(node *scaffui.SingleChildNode[RectangleProps], position smath.Vec, renderer scaffui.Renderer) {
			props := node.Props()
